...
### Quadrat test:
```{r}
grouse_qt <- quadrat.test(grouse_full, nx = 5, ny = 10) # 5(nx) x windows, 10(ny) y windows
...
### Density
```{r}
grouse_density <- density(grouse_full, sigma = 0.05) # density depends on sigma (band width) think hard to pick the right band width
plot(grouse_density)
...
```{r}
r <- seq(0,0.02, by = 0.001) #for csr4326 projection

gfunction <- envelope(grouse_full, fun = Gest, r = r, nsim = 100, nrank = 2) # Sig level of Monte Carlo = 0.04 # nsim: number of simulation
